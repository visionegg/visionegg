...
# Hopefully, the node ID numbers in the name sequence is all that needs to be changed in order to get this module
# working with different Python release versions.
name_sequence = [292, 293, 294, 295, 297, 298, 299, 300, 301, 302, 303, 304, 305]
...
class AST_Visitor:
    def __init__(self, modifying_AST):
        # Flags:
        # Checking assignment name (ie. "x = ..." where we are checking if 'x' == 'assign_name'):
        self.checking_assign_name = 0
        # Setting assignment value (we are changing a found occurrence of "assign_name = x" to "assign_name = assign_val"):
        self.setting_assign_val = 0
        # Index of where the AST visitor is currently at in the 'name_sequence' defined earlier:
        self.name_seq_index = 0
        # AST visitor expects a subtree associated with assignment/"=":
        self.expecting_equals = 0
        # Extracting assignment value (we are extracting the 'x' from a found occurrence of "assign_name = x"):
        self.extracting_assign_val = 0
        # Extracted assignment value:
        self.extracted_val = "not found"
        # Are we modifying an AST? Otherwise we're extracting information from an AST:
    ...
    def traverse(self, AST_sublist, assign_name, assign_val_subtree):
        ...
        # If we have a sub tree of the form [x, [....]]:
            ...
            # If we are somewhere in the 'name_sequence' defined earlier:
                ...
                if self.name_seq_index == len(name_sequence) - 1 and AST_sublist[0] == name_sequence[self.name_seq_index]:
                    if len(AST_sublist[1]) == 3:
                        if self.extracting_assign_val == 1:
                            ...
                            # Enter new mode: AST visitor will check the name associated with the 'name_sequence' to see if it will match 'assign_name':
            ...
            # [a, [b, [c, [d, [e, [...
                ...
                if AST_sublist[0] == 292:
                    if self.setting_assign_val == 1:
                        ...
                        # Enter new mode: AST visitor will check to see if we are progressing through the 'name_sequence' defined earlier:
        ...
        # If we have a sub tree with 3 parent nodes:
            ...
            # If the second parent node is a single element (terminating leaf node):
            if type(AST_sublist[1]) != list:

                # If the current AST visitor mode is set to checking for equality with "assign_name" (ie. "x = ..." where we are checking if 'x' == 'assign_name')::
                if self.checking_assign_name == 1:
                    # If 'x' == 'assign_name' (see above):
                    if AST_sublist[1] == assign_name:
                        # Enter new mode: AST visitor will check to see if the current sub tree is associated with assignment/"=":
                        self.expecting_equals = 1
                    self.checking_assign_name = 0

                # If the current AST visitor mode is set to check if the current sub tree is associated with assignment/"=":
                elif self.expecting_equals == 1:
                    # If the current AST sub tree is associated with assignment/"=":
                    if AST_sublist[1] == '=':
                        if self.modifying_AST == 1:
                            # Enter new mode: AST visitor will change the assignment value to "assign_val_subtree":
                            self.setting_assign_val = 1
                        elif self.modifying_AST == 0:
                            # Enter new mode: AST visitor will extract the assignment value of "assign_name":
                            self.extracting_assign_val = 1
                    self.expecting_equals = 0

                # If the current AST visitor mode is set to assignment value change:
                #elif self.setting_assign_val == 1:
                #    AST_sublist[1] = '%(assign_val)d' % vars()
                #    self.setting_assign_val = 0
        ...
        # For all other types of sub trees, AST visitor will traverse in a depth first search pattern:
        sub_list = []
