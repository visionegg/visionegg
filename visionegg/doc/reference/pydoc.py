...
class HTMLRepr(Repr):
    ...
    def repr(self, object):
        val = Repr.repr(self, object)
        val = re.sub(r' at 0x[0-9a-fA-f]+&gt;','&gt;',val)
        return val
...
<html><head><title>VisionEgg reference: %s</title>
<style type="text/css"><!--
...
<IMG SRC="/cgi-bin/ax.pl?trans.gif" HEIGHT="1" WIDTH="1">
</NOSCRIPT>
...
<td valign=bottom align=left>&nbsp;<br>
  <font color="%s" face="helvetica, arial"><br>%s</font>
</td>
<td valign=bottom align=right>
  <font color="%s" face="helvetica, arial">
  <big><big><strong>
    VisionEgg Library Reference
  </big></big></strong></font>
</td>
</tr>
<tr bgcolor="%s">
<td colspan=2>
  ...
  <center>
  <small>
  This document was produced automatically from the VisionEgg source
  code using the pydoc module of Python.
  <br>
  <a href="http://www.visionegg.org/">www.visionegg.org</a>
</small>
  </center>
    ...
    ''' % (bgcol,fgcol, title, fgcol, bgcol, fgcol)
...
<tr><td bgcolor="%s">%s</td><td>%s</td>''' % (bgcol, marginalia, gap)
    ...
    def docmodule(self, object, name=None, mod=None, *ignored):
        ...
        if hasattr(object, '__version__'):
            ...
            info.append('release %s' % self.escape(version))
...
#        print "realname",realname
        ...
        def spill(msg, attrs, predicate, inher):
##            print "spill! ***********************************"
##            print string.split(msg)[1]
            skip_this = 0
            #print type(msg)
            if msg[:9] == 'Methods i':
                skip_this = 1
                #print "**INHER"
            if msg[:15] == 'Class methods i':
                skip_this = 1
                #print "**INHER"
                
#            if string.split(msg)[1] == 'inherited':
#                skip_this = 1
##                print "msg",msg
##                return []
...
##                print "not skipped"
        ...
        def spillproperties(msg, attrs, predicate, inher):
            ok, attrs = _split_list(attrs, predicate)
            if inher:
                return attrs
            if ok:
        ...
        def spilldata(msg, attrs, predicate, inher):
            ok, attrs = _split_list(attrs, predicate)
            if inher:
                return attrs
            if ok:
                ...
                push(msg)
                for name, kind, homecls, value in ok:
                    if name == "parameters_and_defaults" or name == "constant_parameters_and_defaults":
                        base = self.docparams(getattr(object, name), name, mod)
                    else:
...
#                print "used"
            ...
            new_attrs.append((key, kind, homecls, value))
        ...
        while attrs:
            ...
            attrs = spill("Methods %s" % tag, attrs,
                          lambda t: t[1] == 'method',inher)
            attrs = spill("Class methods %s" % tag, attrs,
                          lambda t: t[1] == 'class method', inher)
            attrs = spill("Static methods %s" % tag, attrs,
                          lambda t: t[1] == 'static method', inher)
            attrs = spillproperties("Properties %s" % tag, attrs,
                                    lambda t: t[1] == 'property', inher)
            attrs = spilldata("Data and non-method functions %s" % tag, attrs,
                              lambda t: t[1] == 'data', inher)
            assert attrs == []
            attrs = inherited
    ...
    def docparams(self, object, name=None, mod=None, *ignored):
        """Produce HTML documentation for a parameters data stucture."""
        lhs = name and '<strong>%s</strong> = {' % name or ''
        my_str = ''
        for pname in object.keys():
            my_str += '<tr><td><tt>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</tt></td><td><strong>%s</strong> : %s,</td></tr>'%(pname,self.repr(object[pname]))
        final_str = '<table><tr><td colspan=2>%s</td></tr>%s<tr><td colspan=2>}</td></table>'%(lhs,my_str)
        return final_str
