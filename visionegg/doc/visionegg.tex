...
\release{0.9.5a0}		% software release, not documentation
\setreleaseinfo{}		% empty for final release
\setshortversion{0.9.5a0}	% major.minor only for software
...
\class{Presentation}, a class which maintains an association between
...
ways to use the \class{Presentation} class, and here they are in
...
stimuli, you are welcome to replace calls to the \method{go} and
\method{run_forever} methods with your own code.  In this case, you
...
``controllers'', instances of the class \class{Controller}.  A
...
the ``center'' parameter of a \class{Target2D} stimulus is updated on
every frame by a function which computes position based upon the
current time.  You can also control parameters without using
